    ...
    CoveApiTestFixture fixture) : ApiTest(output, fixture)
{
    ...
    public async Task GivenOwnerManagedRoleAndContentRule_WhenLifecycleAndAuthorizationRun_ThenPersistenceVisibilityAndDeniedAdminMutationsAreExact()
    {
        ...
        var permissions = await owner.GetRolePermissionsAsync(TestContext.Current.CancellationToken);
        ...
        var created = await owner.CreateRoleAsync(new CreateRoleRequest(roleName, createdDescription, [Permissions.StudiosRead]), TestContext.Current.CancellationToken);
        AssertRole(created, roleName, createdDescription, [Permissions.StudiosRead], expectedUserCount: 0);
        AssertRole(await owner.GetRoleAsync(created.Id, TestContext.Current.CancellationToken), roleName, createdDescription, [Permissions.StudiosRead], expectedUserCount: 0);
        var updated = await owner.UpdateRoleAsync(created.Id, new UpdateRoleRequest(updatedDescription, [Permissions.StudiosRead, Permissions.TagsRead]), TestContext.Current.CancellationToken);
        AssertRole(updated, roleName, updatedDescription, [Permissions.StudiosRead, Permissions.TagsRead], expectedUserCount: 0);
        AssertRole(await owner.GetRoleAsync(created.Id, TestContext.Current.CancellationToken), roleName, updatedDescription, [Permissions.StudiosRead, Permissions.TagsRead], expectedUserCount: 0);
        ...
        AssertRole((await owner.GetRolesAsync(TestContext.Current.CancellationToken)).Should().ContainSingle(role => role.Id == created.Id).Which, roleName, updatedDescription, [Permissions.StudiosRead, Permissions.TagsRead], expectedUserCount: 0);
        (await owner.GetRolesAsync(TestContext.Current.CancellationToken)).Should().NotContain(role => role.Name == $"Forbidden role {suffix}");
        ...
        var studio = await owner.CreateStudioAsync($"Scoped studio {suffix}", TestContext.Current.CancellationToken);
        var scopedUsername = $"scoped-role-user-{suffix}";
        const string scopedPassword = "Scoped role user 123!";
        var scopedUser = await owner.CreateUserAsync(new CreateUserRequest(scopedUsername, scopedPassword, Roles: [roleName]), TestContext.Current.CancellationToken);
        using (var unrestrictedSession = await owner.CreateAuthSessionAsync(scopedUsername, scopedPassword, TestContext.Current.CancellationToken))
            (await unrestrictedSession.Client.GetStudiosAsync(TestContext.Current.CancellationToken)).Select(item => item.Id).Should().Equal(studio.Id);
        ...
        AssertRule((await owner.GetContentRulesAsync(created.Id, TestContext.Current.CancellationToken)).Should().ContainSingle().Which, created.Id, roleName, EntityKinds.Studio, "allow", "all", "{}", "read");
        ...
        AssertRule((await owner.GetContentRulesAsync(created.Id, TestContext.Current.CancellationToken)).Should().ContainSingle().Which, created.Id, roleName, EntityKinds.Studio, "deny", "all", "{\"phase\": \"deny\"}", "read");

        using (var restrictedSession = await owner.CreateAuthSessionAsync(scopedUsername, scopedPassword, TestContext.Current.CancellationToken))
            (await restrictedSession.Client.GetStudiosAsync(TestContext.Current.CancellationToken)).Should().BeEmpty();
        ...
        AssertOverride((await owner.GetEntityOverridesAsync(created.Id, EntityKinds.Studio, TestContext.Current.CancellationToken)).Should().ContainSingle().Which, created.Id, roleName, EntityKinds.Studio, studio.Id.ToString(CultureInfo.InvariantCulture), "allow", "read");
        using (var overrideSession = await owner.CreateAuthSessionAsync(scopedUsername, scopedPassword, TestContext.Current.CancellationToken))
            (await overrideSession.Client.GetStudiosAsync(TestContext.Current.CancellationToken)).Select(item => item.Id).Should().Equal(studio.Id);
        ...
        AssertOverride((await owner.GetEntityOverridesAsync(created.Id, EntityKinds.Studio, TestContext.Current.CancellationToken)).Should().ContainSingle().Which, created.Id, roleName, EntityKinds.Studio, studio.Id.ToString(CultureInfo.InvariantCulture), "allow", "read");
        await owner.DeleteEntityOverrideAsync(createdOverride.Id, TestContext.Current.CancellationToken);
        (await owner.GetEntityOverridesAsync(created.Id, EntityKinds.Studio, TestContext.Current.CancellationToken)).Should().BeEmpty();
        using (var overrideDeletedSession = await owner.CreateAuthSessionAsync(scopedUsername, scopedPassword, TestContext.Current.CancellationToken))
            (await overrideDeletedSession.Client.GetStudiosAsync(TestContext.Current.CancellationToken)).Should().BeEmpty();
        ...
        await owner.DeleteContentRuleAsync(updatedRule.Id, TestContext.Current.CancellationToken);
        (await owner.GetContentRulesAsync(created.Id, TestContext.Current.CancellationToken)).Should().BeEmpty();
        using (var restoredSession = await owner.CreateAuthSessionAsync(scopedUsername, scopedPassword, TestContext.Current.CancellationToken))
            (await restoredSession.Client.GetStudiosAsync(TestContext.Current.CancellationToken)).Select(item => item.Id).Should().Equal(studio.Id);
        ...
        AssertRole(await owner.GetRoleAsync(created.Id, TestContext.Current.CancellationToken), roleName, updatedDescription, [Permissions.StudiosRead, Permissions.TagsRead], expectedUserCount: 1);
        await owner.DeleteRoleAsync(created.Id, TestContext.Current.CancellationToken);
        ...
        (await owner.GetRolesAsync(TestContext.Current.CancellationToken)).Should().NotContain(role => role.Id == created.Id);
        var survivingScopedUser = await owner.GetUserAsync(scopedUser.Id, TestContext.Current.CancellationToken);
