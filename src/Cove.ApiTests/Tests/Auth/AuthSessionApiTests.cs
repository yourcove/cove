    ...
    CoveApiTestFixture fixture) : ApiTest(output, fixture)
{
    ...
    public async Task GivenProvisionedApi_WhenAuthStatusAndEmptyExternalStateAreRead_ThenTheyDescribeTheCurrentUserAndServer()
    {
        ...
        var status = await owner.GetBootstrapStatusAsync();
        var providers = await owner.GetExternalLoginProvidersAsync();
        var links = await owner.GetExternalLinksAsync();
    ...
    public async Task GivenPendingInvite_WhenAnonymousRecipientInspectsAndRedeemsIt_ThenAccountIsCreatedAndTheTokenCannotBeReused()
    {
        ...
        var info = await owner.GetInviteInfoAsync(invite.Token);
        using var redeemed = await owner.RedeemInviteAsync(invite.Token, password, username);
        ...
        var redeemedUser = (await redeemed.Client.GetCurrentUserAsync()).GetProperty("user");
        ...
        (await owner.GetUserAsync(int.Parse(redeemedUser.GetProperty("id").GetString()!))).DisplayName.Should().Be("Invited API user");
        (await owner.TryRedeemInviteStatusAsync(invite.Token, password, username)).Should().Be(HttpStatusCode.Gone);
    }
    ...
    {
        var owner = AsUser();
        (await owner.TryLoginStatusAsync("no-such-user", "wrong password")).Should().Be(HttpStatusCode.Unauthorized);
        (await owner.TryLoginStatusAsync(ApiTestUsers.Eva, "wrong password")).Should().Be(HttpStatusCode.Unauthorized);

        using var initial = await owner.CreateAuthSessionAsync(ApiTestUsers.Eva, ApiTestUsers.Password);
        using var rotated = await owner.RefreshAuthSessionAsync(initial);
        ...
        await rotated.Client.LogoutAuthSessionAsync(rotated);
        (await owner.TryRefreshStatusAsync(rotated.RefreshToken)).Should().Be(HttpStatusCode.Unauthorized);
        ...
        using var revocable = await owner.CreateAuthSessionAsync(ApiTestUsers.Eva, ApiTestUsers.Password);
        await revocable.Client.RevokeSessionsAsync();
        (await owner.TryRefreshStatusAsync(revocable.RefreshToken)).Should().Be(HttpStatusCode.Unauthorized);
        ...
        (await owner.GetCurrentUserAsync()).GetProperty("user").GetProperty("username").GetString().Should().Be(ApiTestUsers.Owner);
    }
    ...
    public async Task GivenMember_WhenPreferencesAndPasswordAreChanged_ThenPreferencesPersistAndOnlyThatMembersOldSessionsAreRevoked()
    {
        ...
        var updated = await eva.UpdateUiPreferencesAsync(preferences);
        var currentUser = await eva.GetCurrentUserAsync();
        ...
        (await eva.TryChangeOwnPasswordStatusAsync("wrong password", "Replacement password 123!")).Should().Be(HttpStatusCode.BadRequest);
        using var oldSession = await owner.CreateAuthSessionAsync(ApiTestUsers.Eva, ApiTestUsers.Password);
        await oldSession.Client.ChangeOwnPasswordAsync(ApiTestUsers.Password, "Replacement password 123!");
        (await owner.TryRefreshStatusAsync(oldSession.RefreshToken)).Should().Be(HttpStatusCode.Unauthorized);
        (await owner.TryLoginStatusAsync(ApiTestUsers.Eva, ApiTestUsers.Password)).Should().Be(HttpStatusCode.Unauthorized);
        using var replacement = await owner.CreateAuthSessionAsync(ApiTestUsers.Eva, "Replacement password 123!");
