    ...
    CoveApiTestFixture fixture) : ApiTest(output, fixture)
{
    ...
    public async Task GivenImageMetadata_WhenCreatedAndRead_ThenRelationshipsAndUserRatingRoundTrip()
    {
        ...
        var owner = AsUser();
        var studio = await owner.CreateStudioAsync($"Image studio {Guid.NewGuid():N}");
        var tag = await owner.CreateTagAsync($"Image tag {Guid.NewGuid():N}");
        var performer = await owner.CreatePerformerAsync(new PerformerBuilder().WithName($"Image performer {Guid.NewGuid():N}").Build());
        var gallery = await owner.CreateGalleryAsync(new GalleryBuilder().WithTitle($"Image gallery {Guid.NewGuid():N}").Build());
        var group = await owner.CreateGroupAsync($"Image group {Guid.NewGuid():N}");
        ...
        var created = await owner.CreateImageAsync(request);
        var retrieved = await AsUser(ApiTestUsers.Eva).GetImageByIdAsync(created.Id);
        var ownerEngagement = await owner.GetEntityEngagementAsync(AffinityHostType.Image, created.Id);
        var memberEngagement = await AsUser(ApiTestUsers.Eva).GetEntityEngagementAsync(AffinityHostType.Image, created.Id);
    ...
    {
        var owner = AsUser();
        var studio = await owner.CreateStudioAsync($"Original image studio {Guid.NewGuid():N}");
        var tag = await owner.CreateTagAsync($"Original image tag {Guid.NewGuid():N}");
        var replacementTag = await owner.CreateTagAsync($"Replacement image tag {Guid.NewGuid():N}");
        var performer = await owner.CreatePerformerAsync(new PerformerBuilder().WithName($"Original image performer {Guid.NewGuid():N}").Build());
        var gallery = await owner.CreateGalleryAsync(new GalleryBuilder().WithTitle($"Original image gallery {Guid.NewGuid():N}").Build());
        var group = await owner.CreateGroupAsync($"Original image group {Guid.NewGuid():N}");
        var image = await owner.CreateImageAsync(new ImageBuilder()
            ...
            .WithUrl("https://image.example/original").WithTag(tag).WithPerformer(performer).WithGallery(gallery).WithGroup(group).Build());
        ...
        var retrieved = await owner.GetImageByIdAsync(image.Id);
    ...
    public async Task GivenMatchingImages_WhenFilteredSortedAndPaged_ThenOnlyTheRequestedPageIsReturned()
    {
        ...
        var owner = AsUser();
        var first = await owner.CreateImageAsync(new ImageBuilder().WithTitle($"A filtered image {suffix}").AsOrganized().Build());
        var second = await owner.CreateImageAsync(new ImageBuilder().WithTitle($"B filtered image {suffix}").AsOrganized().Build());
        await owner.CreateImageAsync(new ImageBuilder().WithTitle($"Excluded image {suffix}").Build());
    ...
    {
        var owner = AsUser();
        var first = await owner.CreateImageAsync($"Aggregate image first {Guid.NewGuid():N}");
        var second = await owner.CreateImageAsync($"Aggregate image second {Guid.NewGuid():N}");
        var excluded = await owner.CreateImageAsync($"Aggregate image excluded {Guid.NewGuid():N}");
        await AsDbUser().AttachImageFileAsync(first.Id, 1_000);
        await AsDbUser().AttachImageFileAsync(second.Id, 2_500);
        await AsDbUser().AttachImageFileAsync(excluded.Id, 9_999);

        var aggregate = await AsUser(ApiTestUsers.Eva).AggregateImagesAsync(new FilteredQueryRequest<ImageFilter> { Ids = [first.Id, second.Id] });
    ...
    {
        var owner = AsUser();
        var originalStudio = await owner.CreateStudioAsync($"Original bulk image studio {Guid.NewGuid():N}");
        var originalTag = await owner.CreateTagAsync($"Original bulk image tag {Guid.NewGuid():N}");
        var originalPerformer = await owner.CreatePerformerAsync(new PerformerBuilder().WithName($"Original bulk image performer {Guid.NewGuid():N}").Build());
        var originalGallery = await owner.CreateGalleryAsync(new GalleryBuilder().WithTitle($"Original bulk image gallery {Guid.NewGuid():N}").Build());
        var replacementTag = await owner.CreateTagAsync($"Replacement bulk image tag {Guid.NewGuid():N}");
        var replacementPerformer = await owner.CreatePerformerAsync(new PerformerBuilder().WithName($"Replacement bulk image performer {Guid.NewGuid():N}").Build());
        var replacementGallery = await owner.CreateGalleryAsync(new GalleryBuilder().WithTitle($"Replacement bulk image gallery {Guid.NewGuid():N}").Build());
        ...
        var control = await owner.CreateImageAsync(new ImageBuilder().WithTitle($"Control bulk image {Guid.NewGuid():N}").WithCode("CONTROL-CODE")
            ...
            .WithTag(originalTag).WithPerformer(originalPerformer).WithGallery(originalGallery).Build());
        await AsUser(ApiTestUsers.Eva).SetImageRatingAsync(control, 17);
        ...
        var updatedCount = await AsUser(ApiTestUsers.Eva).BulkUpdateImagesAsync(request);
        var updated = await Task.WhenAll(selected.Select(item => owner.GetImageByIdAsync(item.Id)));
        var controlAfter = await owner.GetImageByIdAsync(control.Id);
        ...
        var controlEngagement = await AsUser(ApiTestUsers.Eva).GetEntityEngagementAsync(AffinityHostType.Image, control.Id);
    ...
    public async Task GivenImages_WhenOwnerDeletesSelectedRecords_ThenMemberCannotDeleteAndControlsRemain()
    {
        ...
        var single = await owner.CreateImageAsync($"Single delete image {Guid.NewGuid():N}");
        ...
        (await owner.GetImageByIdAsync(single.Id)).Id.Should().Be(single.Id);
        await owner.DeleteImageAsync(single.Id);
        ...
        var first = await owner.CreateImageAsync($"Bulk delete image first {Guid.NewGuid():N}");
        var second = await owner.CreateImageAsync($"Bulk delete image second {Guid.NewGuid():N}");
        var retained = await owner.CreateImageAsync($"Retained image {Guid.NewGuid():N}");
        ...
        foreach (var image in new[] { first, second })
            (await owner.GetImageByIdAsync(image.Id)).Id.Should().Be(image.Id);

        await owner.BulkDeleteImagesAsync(request);
        ...
        (await owner.GetImageByIdAsync(retained.Id)).Id.Should().Be(retained.Id);
    }
}
