    ...
    CoveApiTestFixture fixture) : ApiTest(output, fixture)
{
    ...
    public async Task GivenDeterministicPerformerProvider_WhenSystemNameFragmentAndPreviewRun_ThenResultsAndPermissionsAreExact()
    {
        ...
        var performer = await AsUser().CreatePerformerAsync(new PerformerBuilder()
            ...
            .Build());
        var before = await AsUser().GetPerformerByIdAsync(performer.Id);
        ...
        await AsUser().CreateRoleAsync(new CreateRoleRequest(noReadRoleName, "API test no-permission role", []));
        var noReadUsername = $"performer-scraper-none-{suffix}";
        const string noReadPassword = "Performer scraper no access 123!";
        await AsUser().CreateUserAsync(new CreateUserRequest(noReadUsername, noReadPassword, Roles: [noReadRoleName]));
        using var noReadSession = await AsUser().CreateAuthSessionAsync(noReadUsername, noReadPassword);

        var descriptor = (await viewerSession.Client.GetScrapersAsync())
        ...
        (await AsUser().GetPerformerByIdAsync(performer.Id)).Should().BeEquivalentTo(before);

        var preview = await AsUser(ApiTestUsers.Eva).PreviewPerformerScrapeAsync(performer.Id, previewRequest);
    ...
    {
        var suffix = Guid.NewGuid().ToString("N");
        var scrapedTag = await AsUser().CreateTagAsync("API Test Scraped Tag");
        ...
        var control = await AsUser().CreatePerformerAsync(new PerformerBuilder()
            ...
            .Build());
        var urlBefore = await AsUser().GetPerformerByIdAsync(urlTarget.Id);
        var nameBefore = await AsUser().GetPerformerByIdAsync(nameTarget.Id);
        var controlBefore = await AsUser().GetPerformerByIdAsync(control.Id);
        ...
        await forbiddenName.Should().ThrowAsync<InvalidOperationException>().WithMessage("*returned 403 (Forbidden)*");
        (await AsUser().GetPerformerByIdAsync(urlTarget.Id)).Should().BeEquivalentTo(urlBefore);
        (await AsUser().GetPerformerByIdAsync(nameTarget.Id)).Should().BeEquivalentTo(nameBefore);
        ...
        (await AsUser().GetPerformerByIdAsync(nameTarget.Id)).Should().BeEquivalentTo(nameBefore);
        await AsUser().DeleteEntityOverrideAsync(denyWrite.Id);

        var urlApplied = await AsUser(ApiTestUsers.Eva).ScrapePerformerUrlAsync(
            urlTarget.Id,
            new PerformerScrapeUrlRequestDto(url, false));
        AssertApplied(urlApplied, "API Test URL Performer", url, scrapedTag.Id, "Retained URL alias");
        AssertApplied(
            await AsUser().GetPerformerByIdAsync(urlTarget.Id),
        ...
        var nameApplied = await AsUser(ApiTestUsers.Eva).ScrapePerformerAsync(
            nameTarget.Id,
            new PerformerScrapeRequestDto("name", ScraperId, null, name, false));
        AssertApplied(
            nameApplied,
            $"{name} Scraped",
            "https://api-test.invalid/performers/search-result",
            ...
            "Retained name alias");
        AssertApplied(
            await AsUser().GetPerformerByIdAsync(nameTarget.Id),
            $"{name} Scraped",
            "https://api-test.invalid/performers/search-result",
            scrapedTag.Id,
            "Retained name alias");
        (await AsUser().GetPerformerByIdAsync(control.Id)).Should().BeEquivalentTo(controlBefore);
