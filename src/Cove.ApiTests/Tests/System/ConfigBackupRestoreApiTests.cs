    ...
    CoveApiTestFixture fixture) : ApiTest(output, fixture)
{
    ...
    public async Task GivenSavedTestConfig_WhenOwnerBacksUpAndRestoresIt_ThenFilesPermissionsAndSnapshotsAreExact()
    {
        ...
        try
        {
            await File.WriteAllBytesAsync(configPath, originalConfig);
            ...
            (await File.ReadAllBytesAsync(configPath)).Should().Equal(originalConfig);

            var backup = await AsUser().BackupConfigAsync();
            ...
            (await File.ReadAllBytesAsync(backup.BackupPath)).Should().Equal(originalConfig);
            ...
            await File.WriteAllBytesAsync(configPath, changedConfig);
            await WaitForNextBackupTimestampAsync(backup.Timestamp);
            var beforeRestore = EnumerateConfigBackups(backupDirectory);
            var restored = await AsUser().RestoreConfigAsync(backup.BackupPath);
            ...
            (await File.ReadAllBytesAsync(preRestorePath)).Should().Equal(changedConfig);
            var latest = await AsUser().ReadEndpointAsync(ReadEndpoint.LatestConfigBackup);
