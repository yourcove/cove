    ...
    BlobReferenceTransactionCoordinator? blobReferenceTransactions = null)
{
    ...
    public async Task<EntityNameConflictScanDto> ResolveAllRecommendedAsync(
        string entityType,
        ...
        CancellationToken ct = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(expectedRevision))
            throw new ArgumentException("The scanned conflict-list revision is required.", nameof(expectedRevision));

        EntityNameConflictScanDto? result = null;
        var performerMerges = new List<PerformerMergeResult>();
        var studioMerges = new List<StudioMergeResult>();
        var attempt = 0;
        var executionStrategy = db.Database.CreateExecutionStrategy();
        await executionStrategy.ExecuteAsync(async () =>
        {
            ...
            {
                await using var transaction = await db.Database.BeginTransactionAsync(IsolationLevel.Serializable, ct);
                var initial = await scanner.ScanAsync(entityType, ct);
                if (!string.Equals(initial.Revision, expectedRevision, StringComparison.Ordinal))
                    throw new InvalidOperationException("The conflict list changed. Refresh the scan before applying all recommendations.");

                var blockedReferences = initial.Groups
                    .SelectMany(group => group.Impacts)
                    .Where(impact => groupMergeIds(initial, impact.EntityId))
                    .SelectMany(impact => impact.ExternalReferences)
                    .ToArray();
                if (blockedReferences.Length > 0)
                    throw new EntityMergeBlockedException(
                        entityType,
                        blockedReferences.Sum(reference => reference.RowCount ?? 0),
                        blockedReferences.Select(reference => reference.EntityId).Distinct().Count(),
                        blockedReferences.Any(reference => reference.AccessLimitation != null));

                foreach (var group in initial.Groups)
                {
                    if (group.RecommendedMergeEntityIds.Count == 0)
                        continue;
                    if (entityType == NameConflictEntityTypes.Performer)
                    {
                        var merge = await performerMergeService.MergeWithinTransactionAsync(
                            group.RecommendedSurvivorEntityId,
                            group.RecommendedMergeEntityIds,
                            bypassPerformerVisibility: true,
                            ct);
                        performerMerges.Add(merge);
                    }
                    else
                    {
                        var merge = await studioMergeService.MergeWithinTransactionAsync(
                            group.RecommendedSurvivorEntityId,
                            group.RecommendedMergeEntityIds,
                            bypassStudioVisibility: true,
                            ct);
                        studioMerges.Add(merge);
                    }
                }

                db.ChangeTracker.Clear();
                result = await scanner.ScanAsync(entityType, ct);
                if (result.UnresolvedGroupCount != 0)
                    throw new InvalidOperationException("Recommended cleanup did not produce a conflict-free namespace. No changes were committed.");
                await transaction.CommitAsync(ct);
                if (blobReferenceTransaction != null)
                    await blobReferenceTransaction.CompleteAsync();
            }
        ...
        foreach (var merge in performerMerges)
            performerMergeService.PublishCompletedMerge(merge);
        foreach (var merge in studioMerges)
            studioMergeService.PublishCompletedMerge(merge);
        return result!;

        static bool groupMergeIds(EntityNameConflictScanDto scan, int entityId)
            => scan.Groups.Any(group => group.RecommendedMergeEntityIds.Contains(entityId));
    }
