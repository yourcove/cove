    ...
    ILogger logger)
{
    public async Task<AssetGenerationSummary> GenerateRequestedAssetsAsync(
        ...
        IReadOnlyCollection<string> processedVideoPaths,
        IReadOnlyCollection<string> processedImagePaths,
        IReadOnlyCollection<string> processedAudioPaths,
        IReadOnlyCollection<string> processedTextPaths,
        IReadOnlyCollection<int> changedVideoIds,
        ...
        CancellationToken ct)
    {
        ...
        if (generateVideoAssets && processedVideoPaths.Count > 0)
        {
            ...
            using var representedVideos = new ScanDiskCollection<int>(db, id => id.ToString(System.Globalization.CultureInfo.InvariantCulture), ct: ct);
            await foreach (var candidateFiles in ReadSelectedFileBatchesAsync(db, db.VideoFiles.Include(file => file.ParentFolder).Include(file => file.Fingerprints).Include(file => file.Video), processedVideoPaths, ct))
            {
                ...
                var representativeFileIds = processedVideoFiles
                    ...
                    .Where(group => representedVideos.TryAdd(group.Key!.Value))
                ...
                var videoFiles = processedVideoFiles
                    .Where(file =>
                    {
                        ...
                        var isRepresentative = representativeFileIds.TryGetValue(videoId, out var representativeId) && representativeId == file.Id;
                ...
                await Parallel.ForEachAsync(videoFiles, new ParallelOptions { MaxDegreeOfParallelism = maxParallelism, CancellationToken = ct }, async (videoFile, token) =>
                {
                    ...
                    var contentChanged = changedVideoIds.Contains(videoId);
                    var isRepresentative = representativeFileIds.TryGetValue(videoId, out var representativeId) && representativeId == videoFile.Id;
        ...
        if (generateImageAssets && processedImagePaths.Count > 0)
        {
            ...
            await foreach (var candidateFiles in ReadSelectedFileBatchesAsync(db, db.ImageFiles.Include(file => file.ParentFolder).Include(file => file.Fingerprints), processedImagePaths, ct))
            {
        ...
        if (generateAudioAssets && processedAudioPaths.Count > 0)
        {
            ...
            await foreach (var candidateFiles in ReadSelectedFileBatchesAsync(db, db.AudioFiles.Include(file => file.ParentFolder).Include(file => file.Fingerprints), processedAudioPaths, ct))
            {
        ...
        if (generateTextAssets && processedTextPaths.Count > 0)
        {
            ...
            await foreach (var candidateFiles in ReadSelectedFileBatchesAsync(db, db.TextFiles.Include(file => file.ParentFolder).Include(file => file.Fingerprints), processedTextPaths, ct))
            {
    ...
    private static async IAsyncEnumerable<List<TFile>> ReadSelectedFileBatchesAsync<TFile>(
        CoveContext db, IQueryable<TFile> query, IReadOnlyCollection<string> paths,
        [System.Runtime.CompilerServices.EnumeratorCancellation] CancellationToken ct) where TFile : BaseFileEntity
    {
        using var directories = new ScanDiskCollection<string>(db, path => path, FilesystemPaths.PathComparer, ScanSortKey.Filesystem, FilesystemPaths.PathComparer, ct);
        foreach (var path in paths)
        {
            ct.ThrowIfCancellationRequested();
            var directory = Path.GetDirectoryName(path);
            if (!string.IsNullOrWhiteSpace(directory)) directories.Add(ScanPath.NormalizeStoredFolderPath(directory));
        }
        foreach (var folderPage in directories.Chunk(256))
        {
            int? after = null;
            while (true)
            {
                var pageQuery = query.AsNoTracking().Where(file => file.ParentFolder != null && folderPage.Contains(file.ParentFolder.Path));
                if (after.HasValue) pageQuery = pageQuery.Where(file => file.Id > after.Value);
                var page = await pageQuery.OrderBy(file => file.Id).Take(256)
                    .Select(file => new { file.Id, file.Path }).ToListAsync(ct);
                if (page.Count == 0) break;
                after = page[^1].Id;
                var ids = page.Where(file => paths.Contains(ScanPath.Normalize(FilesystemPaths.ToNativePath(file.Path))))
                    .Select(file => file.Id).ToArray();
                if (ids.Length > 0)
                    yield return await query.AsNoTracking().Where(file => ids.Contains(file.Id)).OrderBy(file => file.Id).ToListAsync(ct);
            }
        }
    }
