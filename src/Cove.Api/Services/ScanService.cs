...
{
    private readonly ScanDiscoveryService _scanDiscoveryService = new(scopeFactory, config, logger);
    private readonly ScanAssetGenerationService _scanAssetGenerationService = new(scopeFactory, fingerprintService, thumbnailService, logger);
    ...
    public string StartScan(ScanOperationOptions? options = null)
    {
        ...
        return jobService.Enqueue("scan", "Scanning library", async (progress, ct) =>
        {
            ...
            var discovery = await _scanDiscoveryService.DiscoverAsync(options, progress, ct);
            ...
            if (files.Count > 0)
            {
                ...
                var assetSummary = await _scanAssetGenerationService.GenerateRequestedAssetsAsync(
                    db,
                    progress,
            ...
            if (directoryScanContext.Enabled)
                await _scanDiscoveryService.PersistDirectoryScanStatesAsync(directoryScanContext, ct);
    ...
    private static string NormalizePath(string path) => ScanPath.Normalize(path);
    private static FileStat GetFileStat(string path) => ScanPath.GetFileStat(path);
    private static DateTime NormalizeFileModTime(DateTime modTime) => ScanPath.NormalizeFileModTime(modTime);
    private static string NormalizeStoredFolderPath(string path) => ScanPath.NormalizeStoredFolderPath(path);
    private static string? TryCanonicalizeStoredFolderPath(string? path) => ScanPath.TryCanonicalizeStoredFolderPath(path);
    private static string? GetParentStoredFolderPath(string storedPath) => ScanPath.GetParentStoredFolderPath(storedPath);
