...
- Administrators now see a persistent upgrade-readiness warning when tag names or aliases collide under Cove 1.3.0's trimmed shared namespace, plus a Settings → Operations → Tag Name Conflicts tool that remains available after cleanup. The tool explains every claim, previews core and extension-owned impact, recommends the most-referenced canonical owner with a lowest-ID tie-breaker, supports a chosen survivor with per-claim merge, alias removal, or rename actions, offers deterministic apply-all cleanup, and refreshes after each transactional operation. Cove 1.3.0 will require remaining conflicts to be resolved from the latest Cove 1.2.x before its enforcement migration can run.
- Cove 1.3.0 upgrade note: the namespace migration will stop before making changes when unresolved groups remain and direct administrators back to the latest Cove 1.2.x cleanup tool. Databases with no conflicts upgrade directly; after preflight, 1.3.0 performs deterministic trimming and alias cleanup only, then validates and enforces the shared namespace.
