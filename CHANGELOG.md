...
- Cove 1.3.0 upgrade note: name-rule enforcement will stop before making changes when unresolved groups remain and direct administrators back to the latest Cove 1.2.x cleanup tool. Databases with no conflicts upgrade directly; after preflight, 1.3.0 performs deterministic trimming and simple cleanup only, then validates and enforces the tag namespace, performer pair, and studio name rules.
