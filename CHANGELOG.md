...
## [Unreleased]
- Rejecting a face suggestion now sticks on the faces list. The cached top suggestion each row shows is a single shared projection that cannot record per-user decisions, so refreshing it used to hand a rejected performer straight back at full confidence while the face's detail view had already moved on to the next-best match. The list now applies the viewer's own decisions to that cached value — showing the same next-best match the detail view shows — and rejected reference-pack matches are dropped from the cache as soon as the rejection is recorded.
