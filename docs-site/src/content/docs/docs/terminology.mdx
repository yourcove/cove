...
| Scene | A scene is a term some metadata providers use for an entry that Cove stores as a video; Cove documentation uses video for the Cove record. |
...
| Segment | A segment is a stored point or time range attached to a video, image, or audio record. |
| Span | A span is a resolved video time range produced from one or more segments by a display profile or query. |
