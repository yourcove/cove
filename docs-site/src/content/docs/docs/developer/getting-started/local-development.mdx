...
Before preparing a Cove pull request, read the [core contribution and version policy](../../contributing/core-development/). A local build reports its position after the latest release, and the same version can be used as an extension compatibility floor.
...
The backend build output also reports the derived Cove version. On a commit two steps after `v1.1.0`, expect `1.1.0-dev.2`; an exact release-tag checkout reports `1.1.0`.
