...
| --- | --- | --- |
| `id` | Required | Stable extension identity. A reverse-domain value is recommended. Direct ZIP and URL installers reject empty ids, rooted paths, `..`, and platform-invalid filename characters. |
...
| `minCoveVersion` | Any Cove version | Declared minimum running Cove version. **Install from ZIP** and **Install from URL** reject an incompatible package, and registry selection excludes or rejects incompatible versions. See the startup-discovery limitation below. |
...
| `registryUrl` | Optional legacy; read if present | Retained for legacy source inference. Current upload, URL, and registry installation source is persisted separately in Cove's installation state. |
