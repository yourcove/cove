...
After the extension has validated the external protocol, call `CompleteAsync` with its id and the
asserted username. Cove accepts only an existing active, unlocked local account and returns a
...
`IMiddlewareExtension` and call `HttpContext.TrySetExtensionUserAssertion`. Establish trust from the
