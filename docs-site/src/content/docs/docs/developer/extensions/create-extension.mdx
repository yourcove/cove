...
The listing must show `extension.json` and `ExampleExtension.dll` without an enclosing `artifacts/extension` directory. Cove accepts the manifest at the archive root or inside one top-level extension directory; a root manifest makes the package easiest to inspect.
...
### Installation succeeds but Hello Cove is absent
