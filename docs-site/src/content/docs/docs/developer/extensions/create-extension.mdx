...
The host compares the running Cove instance version with `minCoveVersion` and refuses to load the extension when the running Cove version is lower. Upgrade Cove or lower the requirement only if you have built and tested against that older host contract.
