...
For the quickest local development loop, upload the built archive without starting an HTTP server:

1. In Cove, open **Settings → Extensions → Discover**.
2. Select **More extension actions** (the three-dot button).
3. Select **Install from ZIP...** and choose the built `.zip` file.
4. Select **Install**.
...
Use **Install from URL...** instead when the package is already hosted somewhere the Cove server can reach. URL installation follows the same archive validation, replacement, and initialization pipeline.
...
<DocsScreenshot
  src="images/docs/thumbnails/install-extension-from-url.webp"
...
The list should contain **Hello Cove**, version `0.1.0`, with **Unverified** and an **Enabled** control. Direct ZIP and URL development packages are unverified because they do not come from Cove's registry. That is the expected observable result for this backend-only example.

## Make a change and reload

To reload the extension during development, rebuild the package and upload the new ZIP again (or install the same URL again):
...
3. Upload the rebuilt ZIP again, or install the same development URL again. Cove replaces the installed package and reloads that extension.
4. Refresh **Installed Extensions** and verify the displayed metadata and enabled state.

When using URL installation, keep the archive URL stable for this local loop. For a release, update `.github/workflows/build.yml` with your extension id and project path, then push a tag whose name matches the manifest version, such as `v0.1.0`.
...
**Install from ZIP** and **Install from URL** reject the package when the running Cove version is lower than `minCoveVersion`; registry installation also filters or rejects incompatible versions. Upgrade Cove or lower the requirement only if you have built and tested against that older host contract. A package copied directly into the extension directory can report the mismatch during startup validation yet still initialize in the current host, so manual placement is not a compatibility bypass or a supported verification result.

Once this minimal package works, add a frontend contribution only when the feature needs one. Use [Cove design principles and guidelines](../../design-guidelines/) to choose a familiar entry point and plan how users reach advanced capabilities. See [UI extension points](../ui-extension-points/) for bundle and component registration.
