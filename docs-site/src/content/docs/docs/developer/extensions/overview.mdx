...
When an extension adopts a host feature from `main` before the next Cove release is known, use that development build as the floor—for example, `1.1.0-dev.42`. The tagged `1.1.0` release does not satisfy that requirement, while later development builds on the same line and later Cove releases do. Cove's `-dev.N` host ordering means “N commits after the base release”; it intentionally differs from ordinary SemVer prerelease ordering. See the [core contribution and version policy](../../contributing/core-development/) for the complete model.
...
- [Frontend runtime API](../frontend-runtime/) for supported imports and shared UI primitives
