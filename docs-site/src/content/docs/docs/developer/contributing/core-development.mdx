...
A release tag such as `v1.1.0` produces Cove version `1.1.0`. A non-tagged build derives its version from the latest reachable release tag and its commit distance:

```text
<latest release>-dev.<commits since that release>
```

For example, the second commit after `v1.1.0` is `1.1.0-dev.2`. This version says where the code came from; it does not predict whether the next release will be `1.1.1`, `1.2.0`, or `2.0.0`. A source tree without an available release tag reports `0.0.0-dev.0` and is not suitable for publication.

Development-build ordering is a Cove host-compatibility rule:

| Running Cove | Minimum required | Compatible | Reason |
| --- | --- | --- | --- |
| `1.1.0` | `1.1.0-dev.2` | No | The release predates the development change. |
| `1.1.0-dev.2` | `1.1.0-dev.2` | Yes | It is the required build. |
| `1.1.0-dev.8` | `1.1.0-dev.2` | Yes | It is later on the same development line. |
| `1.1.1` | `1.1.0-dev.2` | Yes | A later release contains that `main` history. |

Unlike an ordinary SemVer prerelease, `-dev.N` sorts after its base release for this host check because it means “N commits after that release.” Other prerelease labels retain their normal position before the matching release.
...
An extension that requires a Cove feature already merged into `main` should use the earliest development build on which that complete contract was tested. If the feature first exists and works in `1.1.0-dev.42`, use:

```json
{
  "minCoveVersion": "1.1.0-dev.42"
}
```
