...
There is no `/api/v1` compatibility prefix or negotiated API version. The `v1` in the OpenAPI document name identifies the generated document, not a versioned route base. Pin automation to a tested Cove release and compare its OpenAPI contract during upgrades.
