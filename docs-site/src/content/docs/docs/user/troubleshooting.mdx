...
The same log view is available on every deployment at **Settings → System Info → Logs**. Use **Filter** and **Component** to narrow the rows shown in the browser. Changing **Server log level** changes what Cove records. **Trace** is a temporary 15-minute diagnostic session and then returns to the saved level automatically; other levels remain selected until changed.

Trace records detailed per-item decisions and may include local paths, URLs, and library metadata. Enable it only long enough to reproduce the problem once. Cove never intentionally logs credentials, but review and sanitize a log excerpt before sharing it.
