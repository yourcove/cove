...
next:
  link: ../../../tutorial/explore-your-library/
...
The first-run importer also migrates Stash custom fields. Cove creates readable normalized keys, uses **Number** when all values are compatible numbers, uses **JSON** when all values are Cove-compatible JSON and the field is not numeric, and otherwise preserves the source values as **Long Text**. Imported Number fields are ready for filtering and sorting. To query properties inside an imported JSON document, add [typed JSON Pointer targets](../../library/custom-fields/json/) after the import.
