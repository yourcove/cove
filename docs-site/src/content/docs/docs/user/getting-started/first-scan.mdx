...
- For Docker, confirm the media volume is mounted and use its container path, such as `/media`, in Cove. A host path such as `/srv/media` is not visible inside the container unless it is mounted.
