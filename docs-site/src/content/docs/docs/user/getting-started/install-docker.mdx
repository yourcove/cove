...
Unraid templates run `ghcr.io/yourcove/cove-app` and expect a PostgreSQL 18 server with pgvector that you provide. Use a template rather than Compose.

| Template | Use when |
| --- | --- |
| [`cove-cpu.xml`](https://github.com/yourcove/cove/blob/main/docker/unraid/cove-cpu.xml) | CPU only |
| [`cove-nvidia.xml`](https://github.com/yourcove/cove/blob/main/docker/unraid/cove-nvidia.xml) | NVIDIA NVENC |
| [`cove-vaapi.xml`](https://github.com/yourcove/cove/blob/main/docker/unraid/cove-vaapi.xml) | Intel/AMD VAAPI or QSV |
...
The NVIDIA template requires the Nvidia-Driver plugin from Community Applications and already sets `--runtime=nvidia`. Do not add `--gpus all`. The VAAPI template maps `/dev/dri`; switch hardware acceleration to `qsv` in Cove if you want Intel Quick Sync. The app runs as uid `1000`; if it cannot write to `/config`, run `chown -R 1000:1000 /mnt/user/appdata/cove`.
