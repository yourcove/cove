---
title: Install Cove on Linux
description: Install the supported Linux x64 native release and open Cove's first-run setup.
prev:
...
The Linux native release is for 64-bit x86 (`x64`) systems. Cove's automatic managed-PostgreSQL setup currently supports Debian- and Ubuntu-family distributions. For another distribution, use [Docker](../install-docker/) or install PostgreSQL 18 and pgvector locally so a compatible `pg_ctl` is available on `PATH`. For a Linux ARM64 host, use Docker because Cove does not publish a native ARM64 application archive.
