...
[Generated media](../generated-media/) is stored and managed separately from source files. Deleting from the detail page can remove only the video record or also request source-file deletion; bulk deletion additionally offers a generated-files choice. Deleting a parent also deletes its sub-video records, while deleting a sub-video does not delete the parent's source files. Cove retains a source path when a video outside the deletion still references it. Source-file deletion requires a writable [library path](../library-paths/).
