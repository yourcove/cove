...
| **Captions** | Free-text captions metadata retained on the video record and available to filtering; distinct from discovered sidecar caption tracks. |
| **Organized** | User-managed flag indicating that the record has reached the library's desired organization state. |
...
### Relationships and external identity

| Field | Cardinality and behavior |
| --- | --- |
| **Studio** | Zero or one studio. |
| **Tags** | Zero or more tags. Contextual tag applications can also describe a particular performer occurrence or time range without becoming ordinary whole-video tags. |
| **Performers** | Zero or more performers. A dated video can display a performer's age at that date. |
| **Groups** | Zero or more groups. Each membership can carry a **Video #** position used for ordered groups. |
| **Galleries** | Zero or more related galleries. |
| **URLs** | Zero or more external links. |
| **Remote IDs** | Provider endpoint and remote identifier pairs used to reconnect a record to external metadata. |
| **Custom Fields** | Administrator-defined video fields and values. |
| **Parent video** | Present only for a sub-video; the clip start and end define the playable range within the root parent's media. |

Ratings, likes, favorites, play counts, playback sessions, watched duration, and resume position are engagement data. Cove keeps them separately from the shared descriptive fields so the visible values can follow the current signed-in user.

## Files and generated media

The **File Info** tab lists every file attached to the record. A sub-video with no directly attached files resolves the parent video's files for playback.

| File value | Format |
| --- | --- |
| **Path** | Cove-visible filesystem path. In Docker this is the container path, not the host path. |
...
| **Fingerprints** | Available file hashes, such as `oshash`, `md5`, or `phash`. |

Discovered VTT or SRT sidecar caption tracks are associated with individual files and used by the player, but the **File Info** tab does not list them.

Source files are distinct from generated covers, thumbnails, previews, sprites, and other derived assets. Deleting from the detail page can remove only the video record or also request source-file deletion. Bulk deletion additionally offers a separate generated-files choice. Deleting a parent video also deletes its sub-video records, while deleting a sub-video does not delete the parent's source files. Cove also retains a source path when another video record not included in the deletion still references it. Source-file deletion requires a writable media path; it fails on an intentional read-only mount. Keep source media backed up before enabling destructive workflows.
...
- [Terminology](../../terminology/) provides concise definitions of video, generated media, segment, sub-video, and compilation.
