...
export function useVideoQueueNavigation({
  ...
  const openVideo = useCallback((videoId: number) => {
    ...
    if (ids.length > 0 && setQueue) {
      ...
      setQueue(ids, videoId, items.map(toQueueItem), !infinitePageSize ? {
        autoplay,
        ...
        loadNext: lastPage * pageSize < totalCount ? async () => {
          const page = lastPage + 1;
          const response = await queryPage({ ...filter, page });
          lastPage = page;
          return { items: response.items.map(toQueueItem), hasMore: page * pageSize < response.totalCount };
        } : undefined,
      } : { autoplay });
    }
  ...
  const navigateFromList = useCallback((route: any) => {
