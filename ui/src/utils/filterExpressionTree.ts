...
function isEligibleRelatedScopeChild(
...
export function toEditableFilterExpression(
  ...
  const children = expression.children.map((child) =>
    child.group ? { group: toEditableFilterExpression(child.group, criteria, depth + 1) } : child,
  );
  ...
  if (!expression.relatedScope && expression.operator === "AND") {
    const candidates = criteria.filter(
      (candidate) =>
    ...
    if (criterion) {
      ...
      const scope = {
        ...
        relatedScope: {
          ...
          matchMode: legacyDistinct ? ("distinct" as const) : ("reuse" as const),
        },
        children: scopedChildren,
      };
      editable =
        otherChildren.length === 0 ? scope : { operator: "AND", children: [{ group: scope }, ...otherChildren] };
    }
  }
...
export function normalizeFilterExpressionForEditing(
  filter: Record<string, unknown>,
  criteria: CriterionDefinition[] = [],
): Record<string, unknown> {
  const expression = filter[FILTER_EXPRESSION_STATE_KEY] as FilterExpression<Record<string, unknown>> | undefined;
  return expression
    ? { ...filter, [FILTER_EXPRESSION_STATE_KEY]: toEditableFilterExpression(expression, criteria) }
    : filter;
}

export function countFilterExpressionConditions(
  expression: FilterExpression<Record<string, unknown>> | undefined,
): number {
  if (!expression) return 0;
  return expression.children.reduce(
    (count, child) => count + (child.group ? countFilterExpressionConditions(child.group) : child.filter ? 1 : 0),
    0,
  );
}
...
export function isComplexFilterExpression(expression: FilterExpression<Record<string, unknown>> | undefined): boolean {
  return Boolean(
    expression &&
    (expression.relatedScope || expression.operator !== "AND" || expression.children.some((child) => child.group)),
  );
}
...
export function removeExpressionGroup(
  root: FilterExpression<Record<string, unknown>>,
  groupPath: number[],
...
export function getExpressionLeaf(
  root: FilterExpression<Record<string, unknown>>,
  path: number[],
...
function collectExpressionLeaves(
  ...
  return expression.children.flatMap((child, index) =>
    child.filter
      ...
      : child.group
        ? collectExpressionLeaves(child.group, [...path, index])
        : [],
  );
}
...
export function remapExpressionLeafPath(
  ...
  const matchingSourceLeaves = collectExpressionLeaves(source).filter(
    ({ filter }) => JSON.stringify(filter) === signature,
  );
  ...
  return collectExpressionLeaves(destination).filter(({ filter }) => JSON.stringify(filter) === signature)[occurrence]
    ?.path;
}

export function getExpressionGroup(
  root: FilterExpression<Record<string, unknown>>,
  path: number[],
): FilterExpression<Record<string, unknown>> | undefined {
...
export function updateExpressionLeaf(
  root: FilterExpression<Record<string, unknown>>,
  path: number[],
  filter: Record<string, unknown>,
): FilterExpression<Record<string, unknown>> {
  ...
  return repairRelatedScopes(
    replaceExpressionGroup(root, parentPath, (group) => {
...
export function repairRelatedScopes(
  ...
  if (scopedChildren.length < 2)
    return { ...unscoped, operator: "AND", children: [...scopedChildren, ...otherChildren] };
  return {
    ...unscoped,
    operator: "AND",
...
export function removeExpressionLeafAndPrune(
...
function insertExpressionLeafAtMarkedGroup(
  ...
  const children = group.children.map((candidate, index) => {
    ...
    const inserted = insertExpressionLeafAtMarkedGroup(candidate.group as EditableFilterExpression, child, [
      ...path,
      index,
    ]);
...
export function moveExpressionLeaf(
  ...
  const marked = replaceExpressionGroup(
    root,
    destinationPath,
    (group) => ({ ...group, _moveTarget: true }) as EditableFilterExpression,
  ...
  return inserted.insertedPath
    ? { expression: inserted.group, insertedPath: inserted.insertedPath }
    : { expression: root };
}
