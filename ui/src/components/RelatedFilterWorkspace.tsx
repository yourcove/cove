...
export function RelatedFilterWorkspace({
  ...
  const {
    data: savedFilters = [],
    isPending: savedFiltersPending,
    isError: savedFiltersError,
  } = useQuery({
    queryKey: ["saved-filters", entityType],
    queryFn: () => savedFiltersApi.list(entityType),
  });
  const selectedSavedFilterId =
    savedFilters.find((savedFilter) => savedFilter.name === related._savedFilterName)?.id ?? "";
  ...
  const getNestedValue = (nestedCriterion: CriterionDefinition) =>
    criterion.relatedContextCriteria?.some((candidate) => candidate.id === nestedCriterion.id)
  ...
  const activeCriteria = filteredCriteria.filter((candidate) =>
    isCriterionValueValid(getNestedValue(candidate), candidate),
  );
  const inactiveCriteria = filteredCriteria.filter((candidate) => !activeCriteria.includes(candidate));
  const activeConditionCount =
    nestedCriteria.filter((candidate) => isCriterionValueValid(getNestedValue(candidate), candidate)).length +
  ...
  const rovingNavigatorKey =
    navigatorFocusKey && visibleNavigatorKeys.includes(navigatorFocusKey)
      ? navigatorFocusKey
  ...
  const renderCriterionRow = (nestedCriterion: CriterionDefinition) => {
    ...
    return (
      <button
        ref={(element) => {
          if (element) criterionButtonRefs.current.set(nestedCriterion.id, element);
          else criterionButtonRefs.current.delete(nestedCriterion.id);
        }}
  ...
  return (
    <div ref={workspaceRef} className="flex min-h-0 flex-1 flex-col overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-border px-4 py-3 md:flex-row md:items-center md:justify-between md:px-6">
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted">
            {conditionOperator === "or" ? "At least one condition below" : "All conditions below"} must match the same
            related {singular}.
          </p>
        </div>
      ...
      <div className="grid min-h-0 flex-1 overflow-hidden md:grid-cols-[20rem_minmax(0,1fr)]">
        <aside
          className={`${hasEditor ? "hidden md:flex" : "flex"} min-h-0 flex-col border-border md:border-r`}
          aria-label={`${criterion.label} criteria`}
        >
          <div className="space-y-3 border-b border-border p-3 md:p-4">
            ...
            {savedFiltersError ? (
              <p className="text-xs text-red-300">
                Saved filters are unavailable. You can still build the filter here.
              </p>
            ) : null}
          ...
          <div
            ...
            role="tablist"
            aria-label={`Available ${singular} filters`}
          ...
          >
            {showTextSearch ? (
              <section className="mb-4" aria-label="Quick">
                ...
                <button
                  ref={(element) => {
                    if (element) criterionButtonRefs.current.set("search", element);
                    else criterionButtonRefs.current.delete("search");
                  }}
                  type="button"
                  role="tab"
            ...
            {inactiveCriteria.length > 0 ? (
              <section className="mb-4" aria-label={`All ${singular} filters`}>
                <h4 className="px-3 pb-1 text-xs font-semibold uppercase tracking-wide text-muted">
                  All {singular} filters
                </h4>
                <div className="space-y-1">{inactiveCriteria.map(renderCriterionRow)}</div>
              </section>
            ) : null}
            {!showTextSearch && filteredCriteria.length === 0 ? (
              <div className="px-4 py-10 text-center text-sm text-muted">No filters match “{criteriaSearch}”.</div>
            ) : null}
          </div>
        </aside>
        ...
        <main className={`${hasEditor ? "flex" : "hidden md:flex"} min-h-0 min-w-0 flex-col`}>
          {hasEditor ? (
            <div
              role="tabpanel"
              aria-label={
                editingSearch ? "Text search" : editingExistence ? `Any ${singular}` : selectedCriterion?.label
              }
            ...
            >
              <div className="mb-4 flex items-center gap-2 md:hidden">
                <button
                  ...
                  onClick={() => {
                    onSelectionChange(null);
                    window.setTimeout(() => criteriaSearchRef.current?.focus(), 0);
                  }}
              ...
              {editingSearch ? (
                <div className="max-w-2xl space-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-secondary">
                      Match the related {singular}'s name, title, tags, or other searchable text.
                    </p>
                  </div>
                ...
                <div className="max-w-2xl space-y-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-secondary">
                      Require at least one related {singular} without adding another condition.
                    </p>
                  </div>
                  <button
                    ref={matchAnyRef}
                ...
                <div className="max-w-2xl space-y-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {isCriterionValueValid(getNestedValue(selectedCriterion), selectedCriterion) ? (
                      <button
                        type="button"
                        onClick={() => removeNestedCriterion(selectedCriterion)}
                  ...
                  <CriterionEditor
                    ...
                    auxiliaryToggleChecked={
                      selectedCriterion.auxiliaryToggleKey
                        ? Boolean(objectFilter[selectedCriterion.auxiliaryToggleKey])
                        : undefined
                    }
              ...
              <div className="max-w-sm">
                ...
                <h4 className="text-lg font-semibold text-foreground">
                  {relationshipMode === "every"
                    ? `Find ${resultPlural} where every ${singular} matches`
                    : relationshipMode === "none"
                      ? `Find ${resultPlural} where no ${singular} matches`
                      : `Find ${resultPlural} by ${singular}`}
                </h4>
                <p className="mt-1 text-sm text-secondary">
                  {relationshipMode === "every"
