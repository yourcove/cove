...
import { audios, entityImages, faces, galleries, groups, images, performers, segmentLibrary, studios, tags, texts, videos } from "../api/client";
import type { AffinityHostType, Audio, Face, Gallery, Group, GroupItem, Image, Performer, SegmentRecord, Studio, Tag, TextDocument, Video } from "../api/types";
...
type GroupFeedEntityState =
  | { status: "loading" }
  | { status: "error" }
  | { status: "ready"; entity: GroupFeedEntity };
...
export function GroupItemFeed({ groupId, onNavigate, pageSize = DEFAULT_PAGE_SIZE, showGroupHeader = true }: GroupItemFeedProps) {
  ...
  const itemsQuery = useInfiniteQuery({
    ...
    initialPageParam: 1,
    queryFn: ({ pageParam }) => groups.items.page(normalizedGroupId, {
  ...
  if (groupQuery.error || itemsQuery.error) {
    return (
      <div role="alert" className="mx-auto flex min-h-44 w-full max-w-2xl flex-col items-center justify-center gap-3 rounded-xl border border-red-500/25 bg-red-500/5 p-6 text-center">
        <AlertTriangle className="h-7 w-7 text-red-400" />
        <p className="font-medium text-foreground">This group feed could not be loaded.</p>
        <button
          type="button"
          onClick={() => { void groupQuery.refetch(); void itemsQuery.refetch(); }}
          className="rounded border border-border px-3 py-2 text-sm text-foreground hover:border-accent"
        >
          <RotateCcw className="mr-2 inline h-4 w-4" />Retry
        </button>
      </div>
    );
  }
  ...
  if (groupQuery.isLoading || itemsQuery.isLoading) {
    return <div className="mx-auto min-h-64 w-full max-w-[64rem] animate-pulse rounded-xl bg-card/50" aria-label="Loading group feed" />;
  }
      ...
      {showGroupHeader && groupQuery.data ? (
        <div className="mx-auto mb-4 flex w-full max-w-[64rem] flex-wrap items-end justify-between gap-3 px-3 sm:px-4">
          <div>
            ...
            <p className="mt-1 text-sm text-secondary">{totalCount} {totalCount === 1 ? "item" : "items"}</p>
          </div>
          <button type="button" onClick={() => onNavigate({ page: "group", id: normalizedGroupId })} className="rounded-md border border-border px-3 py-2 text-sm text-foreground hover:border-accent">
            Open group
          </button>
        </div>
      ) : null}
      ...
      {items.length === 0 ? (
        ...
        <div className="mx-auto w-full max-w-[64rem] px-3 sm:px-4">
          <VirtualizedInfiniteList
            ...
            loadMore={() => { void itemsQuery.fetchNextPage(); }}
...
function GroupFeedMessage({ children }: { children: string }) {
  return <div className="mx-auto flex min-h-40 w-full max-w-[64rem] items-center justify-center rounded-xl border border-dashed border-border bg-card/30 px-4 text-center text-sm text-muted">{children}</div>;
}

function useGroupFeedEntities(items: GroupItem[], hasPermission: (permission: string) => boolean, principalKey: string) {
  ...
  const queries = useQueries({ queries: queryItems.map(({ item, host }) => createGroupFeedEntityQuery(item, host, principalKey)) });
  ...
  return {
    ...
    queryFn: async (): Promise<GroupFeedEntity> => {
      switch (host.resource) {
        case "video": return { type: "video", value: await videos.get(host.id) };
        case "image": return { type: "image", value: await images.get(host.id) };
        case "audio": return { type: "audio", value: await audios.get(host.id) };
        case "text": return { type: "text", value: await texts.get(host.id) };
        case "group": return { type: "group", value: await groups.get(host.id) };
        case "performer": return { type: "performer", value: await performers.get(host.id) };
        case "studio": return { type: "studio", value: await studios.get(host.id) };
        case "tag": return { type: "tag", value: await tags.get(host.id) };
        case "gallery": return { type: "gallery", value: await galleries.get(host.id) };
        case "face": return { type: "face", value: await faces.get(host.id) };
        case "segment": {
...
function GroupFeedItemCard({ item, entityState, active, principalKey, onNavigate }: {
  item: GroupItem;
  ...
  const { engagement, rating, setRating, ratingPending } = useEntityEngagement(host?.hostType ?? "video", host?.id ?? 0, {
    enabled: canEngage,
  ...
  const details = readable ? item.notes || getGroupFeedDetails(entity) : loading ? undefined : "This item is not available to the current account.";
  const route = readable ? host.route : undefined;
  const open = () => { if (route) onNavigate(route); };

  return (
    <FeedCardFrame
      dataAttribute={{ "data-feed-group-item": "true" }}
      identity={<FeedIdentityBadge>{readable ? host.kindLabel : "Item"}</FeedIdentityBadge>}
      header={readable ? <><span>Item {item.orderIndex + 1}</span>{item.kind === "videoRange" && item.startSec != null ? <span>Starts at {formatClock(item.startSec)}</span> : null}</> : <span>{loading ? "Loading item" : "Unavailable item"}</span>}
...
function renderGroupFeedMedia(
  ...
  if (!entity || state?.status === "error") {
    return <div className="flex aspect-video items-center justify-center rounded-2xl border border-border bg-card/60 text-muted"><Layers className="h-12 w-12" /></div>;
  }

  if (entity.type === "video") return <GroupVideoFeedMedia item={item} video={entity.value} active={active} onOpen={open} />;
  ...
  return (
    <div className="mx-auto w-full max-w-xl px-2 py-1">
      {renderEntityTile(entity, engagement, onNavigate, open)}
    </div>
  );
}

function GroupVideoFeedMedia({ item, video, active, onOpen }: { item: GroupItem; video: Video; active: boolean; onOpen: () => void }) {
  ...
  const duration = item.kind === "videoRange" && item.startSec != null && item.endSec != null
    ? Math.max(0, item.endSec - item.startSec)
    : file?.duration ?? 0;
  const startTime = item.kind === "videoRange" && item.startSec != null
    ? item.startSec
  ...
  const media = (
    <WallMediaCard
      ...
      style={mediaStyle}
      className={mediaStyle ? "h-full w-full bg-transparent" : "overflow-hidden rounded-2xl border border-border/70 bg-black/95 shadow-[0_18px_40px_rgba(0,0,0,0.35)]"}
    />
  );
  const clickable = <button type="button" onClick={onOpen} className="block w-full cursor-pointer text-left" aria-label={`Open ${title}`}>{media}</button>;
...
function renderEntityTile(entity: GroupFeedEntity, engagement: ReturnType<typeof useEntityEngagement>["engagement"], onNavigate: GroupItemFeedProps["onNavigate"], onOpen: () => void) {
  const normalizedEngagement = engagement ?? undefined;
  switch (entity.type) {
    case "audio": return <AudioTile audio={entity.value} engagement={normalizedEngagement} onClick={onOpen} onNavigate={onNavigate} />;
    case "text": return <TextTile text={entity.value} engagement={normalizedEngagement} onClick={onOpen} onNavigate={onNavigate} />;
    case "group": return <GroupTile group={entity.value} engagement={normalizedEngagement} onClick={onOpen} onNavigate={onNavigate} />;
    case "performer": return <PerformerTile performer={entity.value} engagement={normalizedEngagement} onClick={onOpen} onNavigate={onNavigate} />;
    case "studio": return <StudioTile studio={entity.value} engagement={normalizedEngagement} onClick={onOpen} onNavigate={onNavigate} />;
    case "tag": return <TagTile tag={entity.value} engagement={normalizedEngagement} onClick={onOpen} onNavigate={onNavigate} />;
    case "gallery": return <GalleryTile gallery={entity.value} engagement={normalizedEngagement} onClick={onOpen} onNavigate={onNavigate} />;
    case "face": return <FaceTile face={entity.value} onClick={onOpen} />;
    case "segment": return <SegmentTile segment={entity.value} onClick={onOpen} />;
    case "video": return <div className="flex aspect-video items-center justify-center rounded-xl bg-card"><Film className="h-10 w-10 text-muted" /></div>;
    case "image": return <div className="flex aspect-video items-center justify-center rounded-xl bg-card"><ImageIcon className="h-10 w-10 text-muted" /></div>;
  }
}
...
function resolveGroupFeedHost(item: GroupItem): GroupFeedHost | null {
  ...
  const route = { page: resource, id, ...(resource === "video" && item.startSec && item.startSec > 0 ? { seekTo: item.startSec } : {}) };
...
function getGroupFeedTitle(item: GroupItem, entity?: GroupFeedEntity) {
  ...
  switch (entity.type) {
    case "video": return entity.value.title || entity.value.files[0]?.basename || `Video ${entity.value.id}`;
    case "image": return getImageDisplayTitle(entity.value);
    case "audio": return getAudioDisplayTitle(entity.value);
    case "text": return getTextDisplayTitle(entity.value);
    case "group": return entity.value.name;
    case "performer": return entity.value.name;
    case "studio": return entity.value.name;
    case "tag": return entity.value.name;
    case "gallery": return getGalleryDisplayTitle(entity.value);
    case "face": return entity.value.label || `Face ${entity.value.id}`;
    case "segment": return entity.value.title || `Segment ${entity.value.id}`;
  }
}
