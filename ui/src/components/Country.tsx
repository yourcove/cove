...
export function CountryLabel({ value, className = "" }: { value?: string | null; className?: string }) {
  ...
  if (!option?.code) return <span className={className} title={value}>{value}</span>;
  const name = localizedCountryName(option, language);
  return <span className={`inline-flex items-center gap-1 whitespace-nowrap ${className}`.trim()} title={`${name} (${option.code})`}><span aria-hidden="true">{countryFlag(option.code)}</span><span>{name}</span></span>;
}
...
export function CountryFlag({ value, className = "" }: { value?: string | null; className?: string }) {
  ...
  const name = localizedCountryName(option, language);
  return <span className={className} title={name} aria-label={name}>{countryFlag(option.code)}</span>;
}
...
export function CountrySelect({ value = "", onChange, placeholder = "Search countries or enter a custom value…", allowClear = true, className = "" }: CountrySelectProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter(option => option.value.toLocaleLowerCase().includes(normalizedQuery)
  ...
  const exactOption = data.find(option => option.value.toLocaleLowerCase() === normalizedQuery
  ...
  return (
    <div ref={rootRef} className={`relative w-full min-w-0 ${className}`.trim()}>
      <div className="flex items-center rounded border border-border bg-card focus-within:border-accent">
        {selected?.code ? <span className="ml-3 w-5 shrink-0 text-base leading-none" aria-hidden="true">{countryFlag(selected.code)}</span> : null}
        <input
          ref={inputRef}
          role="combobox"
          aria-label="Country"
          ...
          onKeyDown={event => {
            if (event.key === "ArrowDown" || event.key === "ArrowUp") {
              ...
              setActiveIndex(current => {
                ...
                return Math.max(0, Math.min(selectableOptionCount - 1, current < 0 ? (direction > 0 ? 0 : selectableOptionCount - 1) : current + direction));
              });
            } else if (event.key === "Enter" && (query.trim() || activeIndex >= 0)) {
              event.preventDefault();
              choose(activeIndex >= 0 && activeIndex < filtered.length ? filtered[activeIndex].value : (exactOption?.value ?? query));
            }
            if (event.key === "Escape") { setOpen(false); setQuery(""); setActiveIndex(-1); }
          }}
          className="min-w-0 flex-1 bg-transparent px-3 py-2 text-sm text-foreground outline-none"
        />
        {allowClear && value ? <button type="button" aria-label="Clear country" className="p-1 text-muted hover:text-foreground" onClick={() => choose("")}><X className="h-3.5 w-3.5" /></button> : null}
        <button type="button" aria-label="Show countries" className="p-2 text-muted hover:text-foreground" onClick={() => { setOpen(current => !current); inputRef.current?.focus(); }}><ChevronDown className="h-4 w-4" /></button>
      </div>
      {open ? (
        <AutocompleteDropdown anchorRef={rootRef} containerRef={dropdownRef} id={listboxId} role="listbox" aria-label="Country options" className="rounded border border-border bg-surface">
          {isLoading ? <div className="px-3 py-2 text-xs text-muted">Loading countries…</div> : null}
          {filtered.map((option, index) => {
            const name = localizedCountryName(option, language);
            return <button id={`${listboxId}-${index}`} key={option.value} type="button" role="option" aria-selected={option.value === value} className={`flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-card ${activeIndex === index ? "bg-card" : ""}`} onMouseEnter={() => setActiveIndex(index)} onClick={() => choose(option.value)}>
              <span className="w-5 shrink-0" aria-hidden="true">{countryFlag(option.code)}</span>
              <span className="min-w-0 flex-1 truncate">{name}</span>
              {option.value === value ? <Check className="h-3.5 w-3.5 text-accent" /> : null}
            </button>;
          })}
          {showCustomOption ? <button id={`${listboxId}-${filtered.length}`} type="button" role="option" aria-label={`${query.trim()} Custom value`} aria-selected="false" className={`flex w-full items-center gap-2 px-3 py-2 text-left text-sm hover:bg-card ${activeIndex === filtered.length ? "bg-card" : ""}`} onMouseEnter={() => setActiveIndex(filtered.length)} onClick={() => choose(query)}><span className="min-w-0 flex-1 truncate">{query.trim()}</span><span className="shrink-0 text-xs text-muted">Custom value</span></button> : null}
