...
export function ActiveObjectFilterChips({ criteriaDefinitions, objectFilter, onRemove, onClearAll, customFilterSections, className = "" }: ActiveObjectFilterChipsProps) {
  const activeEntityTypes = useMemo(() => {
    const types = new Set<string>();
    for (const key of Object.keys(objectFilter)) {
      const def = criteriaDefinitions.find((item) => item.id === key || item.filterKey === key);
  ...
  return (
    <div className={`mx-1 mt-1 flex flex-wrap items-center gap-1.5 rounded-lg border border-border bg-surface/50 px-3 py-1.5 ${className}`}>
      {Object.entries(objectFilter).map(([key, value]) => {
        ...
        return (
          <button key={key} type="button" onClick={() => onRemove(key)} className="group flex items-center gap-1 rounded-full border border-border bg-card px-2.5 py-0.5 text-xs text-foreground transition-colors hover:border-red-400 hover:text-red-300" title={`Remove filter: ${label}`}>
