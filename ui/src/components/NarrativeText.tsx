...
const COMPONENTS = {
  h1: (props: ComponentPropsWithoutRef<"h1">) => (
    <h1 className="mt-5 text-2xl font-semibold tracking-tight text-foreground first:mt-0" {...props} />
  ),
  h2: (props: ComponentPropsWithoutRef<"h2">) => (
    <h2 className="mt-5 text-xl font-semibold tracking-tight text-foreground first:mt-0" {...props} />
  ),
  h3: (props: ComponentPropsWithoutRef<"h3">) => (
    <h3 className="mt-4 text-lg font-semibold text-foreground first:mt-0" {...props} />
  ),
  ...
  h6: (props: ComponentPropsWithoutRef<"h6">) => (
    <h6 className="mt-3 text-xs font-semibold uppercase tracking-wide text-muted first:mt-0" {...props} />
  ),
  ...
  blockquote: (props: ComponentPropsWithoutRef<"blockquote">) => (
    <blockquote className="mt-3 border-l-4 border-accent/40 bg-card/60 px-4 py-2 italic first:mt-0" {...props} />
  ),
  code: ({ className, ...props }: ComponentPropsWithoutRef<"code">) => (
    <code
      className={["rounded bg-card px-1 py-0.5 font-mono text-[0.92em] text-foreground", className]
        .filter(Boolean)
        .join(" ")}
      {...props}
    />
  ...
  pre: (props: ComponentPropsWithoutRef<"pre">) => (
    <pre
      className="mt-3 overflow-x-auto rounded-lg border border-border bg-slate-950/95 p-3 text-slate-100 first:mt-0"
      {...props}
    />
  ),
  a: ({ href, children, ...props }: ComponentPropsWithoutRef<"a">) => {
    ...
    return safeHref ? (
      <a
        ...
        className="text-accent underline decoration-accent/40 underline-offset-4 hover:text-accent/80"
        {...props}
      >
        {children}
      </a>
