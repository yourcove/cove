...
export const GalleryScrubThumbnail = forwardRef<GalleryScrubThumbnailHandle, GalleryScrubThumbnailProps>(function GalleryScrubThumbnail({ gallery, coverUrl, coverWidth = 640, fit = "cover", alt = "", enableScrubbing = true }, ref) {
  const queryClient = useQueryClient();
  const containerRef = useRef<HTMLDivElement>(null);
  const resolvedCoverUrl = coverUrl === undefined ? gallery.coverPath ?? galleries.coverUrl(gallery.id, gallery.updatedAt, coverWidth) : coverUrl;
  const [pendingUrl, setPendingUrl] = useState<string | null>(null);
  const [displayedUrl, setDisplayedUrl] = useState<string | null>(null);
  const [activeIndex, setActiveIndex] = useState<number | null>(null);
  const requestSequence = useRef(0);
  const requestedIndex = useRef<number | null>(null);
  const canScrub = enableScrubbing && gallery.imageCount > 1 && window.matchMedia?.("(hover: hover) and (pointer: fine)").matches !== false;
  ...
  const updatePreviewAt = useCallback((clientX: number, clientY: number) => {
    if (!canScrub) return;
  ...
  const updatePreview = useCallback((event: MouseEvent<HTMLDivElement>) => {
  ...
  return (
    ...
    >
      {!displayedUrl && resolvedCoverUrl ? (
        <>
          <img src={resolvedCoverUrl} alt={alt} className={`h-full w-full ${objectFitClass}`} loading="lazy" onError={(event) => { const image = event.currentTarget; image.style.display = "none"; const fallback = image.nextElementSibling as HTMLElement | null; if (fallback) fallback.style.display = "flex"; }} />
          <div className="hidden h-full w-full items-center justify-center text-muted"><FolderOpen className="h-7 w-7" /></div>
        </>
      ) : !displayedUrl ? (
        <div className="flex h-full w-full items-center justify-center text-muted"><FolderOpen className="h-7 w-7" /></div>
      ) : null}
      {displayedUrl ? (
        <img
          src={displayedUrl}
      ...
      {pendingUrl && pendingUrl !== displayedUrl ? (
        <img
          ...
          draggable={false}
          onLoad={() => { setDisplayedUrl(pendingUrl); setPendingUrl(null); }}
      ...
      {activeIndex != null ? (
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[8] h-10">
          <div
            ...
            style={{ left: `${Math.min(88, Math.max(12, (activeIndex / Math.max(1, gallery.imageCount - 1)) * 100))}%` }}
          >
            {activeIndex + 1} / {gallery.imageCount}
          </div>
          <div className="absolute inset-x-1 bottom-1 h-1 rounded-full bg-black/55">
            <div className="h-full rounded-full bg-accent" style={{ width: `${(activeIndex / Math.max(1, gallery.imageCount - 1)) * 100}%` }} />
          </div>
        </div>
      ) : null}
    </div>
