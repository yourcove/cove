...
export function PerformerExternalLinks({ remoteIds, urls, metadataServers }: Props) {
  ...
  const visibleUrls = expanded ? urls : urls.slice(0, Math.max(0, COLLAPSED_LINK_COUNT - remoteIds.length));

  return (
    <>
      <div className="flex flex-wrap gap-2">
        <MetadataServerLinks remoteIds={visibleRemoteIds} entityType="performers" metadataServers={metadataServers} />
