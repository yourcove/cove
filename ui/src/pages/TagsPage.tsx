...
function TagCard({ tag, onClick, onNavigate, selected, onSelect, selecting }: { tag: Tag; onClick: () => void; onNavigate: (r: any) => void; selected?: boolean; onSelect?: () => void; selecting?: boolean }) {
  ...
  return (
    <div className={`entity-card bg-card rounded overflow-hidden border hover:border-accent/60 transition-all cursor-pointer group ${selected ? "border-accent ring-2 ring-accent" : "border-border"}`} onClick={onClick}>
