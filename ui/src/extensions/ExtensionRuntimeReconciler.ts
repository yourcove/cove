...
function resolveBundle<TComponent, TActionHandler>(moduleNamespace: unknown): ResolvedBundle<TComponent, TActionHandler> {
  ...
  const handlerExports = definition.actionHandlers === undefined
    ? definition.handlers
...
function validateDescriptors(descriptors: ExtensionRuntimeBundleDescriptor[]) {
  ...
  for (const descriptor of descriptors) {
    const extensionId = typeof descriptor.extensionId === "string"
      ? descriptor.extensionId.trim()
...
function createIdempotentCleanup(
  onUnload: (() => void | Promise<void>) | undefined,
  unregister: () => void,
) {
...
export function createExtensionRuntimeReconciler<TComponent = unknown, TActionHandler = unknown>(
  ...
  const reconcile = (
    ...
    const staged = new Map<ExtensionRuntimeOwner, {
      descriptor: ExtensionRuntimeBundleDescriptor;
      bundle: ResolvedBundle<TComponent, TActionHandler>;
    }>();
    ...
    const stagedEntries = await Promise.all([...desired.values()].map(async (descriptor) => {
